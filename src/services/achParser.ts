...
export class AchParser {
  ...
  private static handleLine(line: string, achInput: AchFile, linenum: number) {
    ...
    if (line[0] === "1") ach.fileHeader = RecordHeader.parseLine(line);
      ...
      if (!parseFunc) {
        const m = `AchParser: 'error'(line ${linenum}): standard entry class '${
          batch.standardEntryClass
        }' is not valid.`;
        throw new Error(m);
      }
      ...
      const batch = ach.batches[ach.batches.length - 1];
      batch.entries[
        ach.batches[ach.batches.length - 1].entries.length - 1
      ].addenda.push(parseFunc(line));
    } else if (line[0] === "8") {
      const batch = ach.batches[ach.batches.length - 1];
      ...
      const m = `AchParser: 'error'(line ${linenum}): record type code '${
        line[0]
      }' is not valid.`;
      throw new Error(m);
    }
