...
describe('Unit AchParser Tests', () => {
  ...
  it('should fail when standard entry class is invalid', async () => {
    try {
      const ach = await AchParser.parseAchFile(resolve('src', 'fixtures', 'basicInvalidSEC.ach'));
    } catch (err) {
  ...
  it('should fail when record type code is invalid', async () => {
    try {
      const ach = await AchParser.parseAchFile(resolve('src', 'fixtures', 'basicInvalidRTC.ach'));
    } catch (err) {
  ...
  it('should fail when header record is invalid', async () => {
    try {
      const ach = await AchParser.parseAchFile(resolve('src', 'fixtures', 'basicInvalidHeader.ach'));
    } catch (err) {
